...
function StatCard({ icon, label, value, sub }: StatCardProps) {
  return (
    <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 flex items-start gap-3 min-w-0">
      <span className="text-2xl shrink-0">{icon}</span>
      <div className="min-w-0">
        <p className="text-xl font-bold tabular-nums text-neutral-900 dark:text-white truncate">{value}</p>
        {sub && <p className="text-xs text-neutral-400 truncate mt-0.5">{sub}</p>}
...
function AchievementBadge({ ach }: { ach: { icon: string; name: string; description: string; earned: boolean; xp_reward: number } }) {
  return (
    ...
    >
      ...
      <span className="text-xs font-medium text-neutral-700 dark:text-neutral-300 leading-tight truncate w-full text-center">{ach.name}</span>
...
export function DashboardPage() {
  ...
  const { data: stats } = useQuery({
    queryKey: ['my-stats'],
    queryFn: () => sessionService.getMyStats(),
  })
    ...
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
      ...
      <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <StatCard
          icon="⚡"
          label="Best WPM"
          value={stats?.best_wpm ? `${Math.round(stats.best_wpm)}` : '—'}
          sub={stats?.average_wpm ? `Avg: ${Math.round(stats.average_wpm)}` : 'all time'}
        />
        <StatCard
          icon="🎯"
          label="Accuracy"
          value={stats?.average_accuracy ? `${stats.average_accuracy.toFixed(1)}%` : '—'}
          sub="average"
        />
        <StatCard
          icon="🔥"
      ...
      {stats && (
        <div className="flex gap-4 text-sm text-neutral-500 dark:text-neutral-400">
          <span><strong className="text-neutral-800 dark:text-neutral-200">{stats.total_sessions}</strong> sessions</span>
          <span><strong className="text-neutral-800 dark:text-neutral-200">{Math.round((stats.total_time_seconds ?? 0) / 60)}</strong> minutes typed</span>
        </div>
      )}
          ...
          <div className="h-56 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 bg-white dark:bg-neutral-900">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={wpmHistory.data} margin={{ top: 5, right: 20, left: -10, bottom: 0 }}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={d => d.slice(5)} stroke="currentColor" strokeOpacity={0.3} />
      ...
      <section>
        <h2 className="text-lg font-semibold text-neutral-800 dark:text-neutral-100 mb-3">
          Achievements
          {earnedCount > 0 && <span className="ml-2 text-sm text-yellow-500">{earnedCount} earned</span>}
        </h2>
        {achLoading ? (
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                {recentSessions.items.map(s => (
                  <tr key={s.id} className="bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-semibold tabular-nums text-emerald-600">{Math.round(s.wpm)}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-neutral-600 dark:text-neutral-300">{s.accuracy.toFixed(1)}%</td>
                    <td className="px-4 py-3 text-right tabular-nums text-neutral-400 hidden sm:table-cell">{Math.round(s.duration_seconds)}s</td>
                  </tr>
                ))}
              </tbody>
            </table>
